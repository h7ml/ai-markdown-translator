...
`ai-markdown-translator`는 OpenAI의 언어 모델을 사용하여 Markdown 파일을 한 언어에서 다른 언어로 번역하는 명령줄 도구입니다. 콘텐츠를 번역하는 동안 Markdown 구문을 보존합니다.

## 기능

- OpenAI 모델에서 지원하는 모든 언어로 Markdown 파일 번역
- 번역 중 Markdown 구문 보존
- 재귀 디렉토리 번역 지원
...
- `format`: `src` 디렉터리 내 다양한 파일 유형에 대해 Prettier를 사용하여 코드를 포맷합니다.
- `format:check`: `src` 디렉터리 내 다양한 파일 유형에 대해 변경 없이 코드 포맷을 점검합니다.
- `postbuild`: 컴파일된 `index.js` 파일을 실행 가능하게 만듭니다.
- `changelog`: 일반 커밋에 따라 변경 로그를 생성합니다.
- `version`: 버전 관리를 위해 변경 로그를 업데이트하고 커밋을 위한 스테이징을 합니다.
- `test`: 프로젝트를 빌드하고 테스트를 실행합니다.

## 사용법

Node.js, `npx` 또는 패키징된 경우 독립 실행 파일로 CLI 도구를 실행할 수 있습니다.

### Node.js 사용하기

```bash
node dist/index.js --input <입력파일> --output <출력파일> --language <목표언어> [옵션]
```

### npx 사용하기

```bash
npx ai-markdown-translator -i <입력파일> -o <출력파일> -l <목표언어> [옵션]
```
...
```bash
./ai-markdown-translator --input <입력파일> --output <출력파일> --language <목표언어> [옵션]
```

## 옵션

- `--input`, `-i`: 입력 Markdown 파일 또는 디렉터리 (`--url`의 대안). 번역하려는 Markdown 파일이나 디렉터리의 경로를 지정할 수 있습니다.
- `--url`, `-u`: 번역할 Markdown 파일의 URL (`--input`의 대안). 번역하려는 Markdown 파일의 직접 링크를 제공하는 옵션입니다.
- `--extension`, `-e`: 번역할 파일 확장자 지정 (예: `md`). 제공하지 않으면 모든 파일이 처리됩니다. 파일의 확장자에 따라 번역할 파일을 필터링하는 옵션입니다.
- `--rename`: 파일 이름을 수정할지 여부. true인 경우 출력 파일 이름은 `<원본 파일 이름>-translated.<확장자>`로 지정됩니다. 번역된 파일 이름에 접미사를 추가할지 여부를 지정하는 옵션입니다.
- `--output`, `-o`: 출력 Markdown 파일 (제공하지 않으면 기본적으로 입력 파일 이름 사용). 번역된 콘텐츠가 저장될 출력 파일의 이름을 지정하는 옵션입니다.
- `--language`, `-l`: 번역할 대상 언어 (필수). Markdown 내용을 번역하려는 언어를 지정하는 옵션입니다.
- `--openai-url`: OpenAI API URL (기본값: `OPENAI_URL` 환경 변수 사용). 필요한 경우 OpenAI API용 사용자 지정 URL을 지정하는 옵션입니다.
- `--api-key`: OpenAI API 키 (기본값: `API_KEY` 환경 변수 사용). 인증을 위해 OpenAI API 키를 제공하는 옵션입니다.
- `--model`: 사용할 OpenAI 모델 (기본값: `MODEL` 환경 변수 또는 `gpt-3.5-turbo` 사용). 번역에 사용할 OpenAI 모델을 지정하는 옵션입니다.
- `--help`, `-h`: 도움말 표시. 명령줄 도구에 대한 도움말 정보를 표시하는 옵션입니다.
- `--show-version`, `-v`: 버전 표시. 도구의 현재 버전을 표시하는 옵션입니다.
...
- `--log-dir`: 로그 디렉터리 지정 (기본값: `<project_root>/log`). 모든 로그 파일이 저장될 디렉터리입니다.
- `--locale`: 로깅 메시지 언어 설정 (기본값: 'zh'). 로깅 메시지의 언어를 지정하는 옵션입니다 (선택지: 'en', 'zh', 'ko').
- `--retry-count`: 실패한 번역을 위한 재시도 횟수 (기본값: 3). 번역기가 실패한 번역을 재시도하는 횟수입니다.
- `--retry-delay`: 재시도 사이의 지연 시간(초) (기본값: 10). 재시도 사이의 대기 시간입니다.
- `--path`, `-p`: 디렉터리 구조 표시 (기본값: 현재 스크립트 디렉터리). 지정된 디렉터리 구조의 트리 뷰를 표시합니다.
...
프로젝트 루트의 `.env` 파일에 설정하거나 셸에서 내보낼 수 있습니다.
...
4. **URL의 Markdown 콘텐츠 번역하기:**

```bash
npx ai-markdown-translator -u https://gitee.com/h7ml/ai-markdown-translator/raw/main/README.md -o output.md -l "Italian"
```

5. **디렉터리 내 모든 Markdown 파일을 번역하고 이름 변경하기:**
...
- **문제**: [문제 신고하기](https://github.com/h7ml/ai-markdown-translator/issues)
...
문제가 발생하거나 질문이 있는 경우 이 리포지토리에 문제를 열어 주세요.