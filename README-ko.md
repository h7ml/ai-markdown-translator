...
`ai-markdown-translator`는 OpenAI의 언어 모델을 사용하여 Markdown 파일을 한 언어에서 다른 언어로 번역하는 명령 줄 도구입니다. 콘텐츠를 번역하면서 Markdown 구문을 유지합니다.

## 기능

- OpenAI 모델에서 지원하는 모든 언어로 Markdown 파일을 번역
- 번역 중 Markdown 구문 유지
- 재귀 디렉터리 번역 지원
...
- `format`: `src` 디렉터리의 다양한 파일 형식을 위해 Prettier를 사용하여 코드를 형식화합니다.
- `format:check`: `src` 디렉터리의 다양한 파일 형식에 대해 변경하지 않고도 코드 형식을 확인합니다.
- `postbuild`: 컴파일된 `index.js` 파일을 실행 가능하게 만듭니다.
- `changelog`: 전통적인 커밋에 따라 변경 기록을 생성합니다.
- `version`: 버전 관리를 할 때 변경 로그를 업데이트하고 커밋 준비를 합니다.
...
- `--input`, `-i`: 입력 Markdown 파일 또는 디렉터리 (`--url`의 대체). 번역하려는 Markdown 파일 또는 디렉터리의 경로를 지정합니다.
- `--url`, `-u`: 번역하려는 Markdown 파일의 URL (`--input`의 대체). 번역하려는 Markdown 파일의 직접 링크를 제공할 수 있습니다.
- `--extension`, `-e`: 번역할 파일 확장자를 지정합니다 (예: `md`). 제공하지 않으면 모든 파일이 처리됩니다. 이 옵션은 확장자에 따라 번역할 파일을 필터링하는 데 사용됩니다.
- `--rename`: 파일 이름을 수정할지 여부. true인 경우 출력 파일의 이름은 `<original-filename>-translated.<extension>`이 됩니다. 이 옵션은 번역된 파일 이름에 접미사를 추가할지 여부를 지정합니다.
- `--output`, `-o`: 출력 Markdown 파일 (지정하지 않으면 기본적으로 입력 파일 이름을 사용). 번역된 콘텐츠가 저장될 출력 파일의 이름을 지정합니다.
- `--language`, `-l`: 번역할 대상 언어 (필수). Markdown 콘텐츠를 번역하려는 언어를 지정합니다.
- `--openai-url`: OpenAI API URL (기본값: `OPENAI_URL` 환경 변수를 사용). 필요한 경우 OpenAI API의 사용자 정의 URL을 지정할 수 있습니다.
- `--api-key`: OpenAI API 키 (기본값: `API_KEY` 환경 변수를 사용). 인증을 위해 OpenAI API 키를 제공하는 데 사용됩니다.
- `--model`: 사용할 OpenAI 모델 (기본값: `MODEL` 환경 변수를 사용하거나 `gpt-3.5-turbo`). 번역에 사용할 OpenAI 모델을 지정할 수 있습니다.
...
- `--log-file`: 로그 파일 경로 지정 (기본값: `<project_root>/log/translator-err.log`). 번역 오류와 실패가 기록되는 파일입니다.
...
- `--retry-count`: 실패한 번역에 대한 재시도 횟수 (기본값: 3). 번역기가 실패한 번역을 재시도할 횟수입니다.
- `--retry-delay`: 재시도 시도 간 대기 시간 (기본값: 10). 재시도 간 대기 시간입니다.
- `--path`, `-p`: 디렉터리 구조 표시 (기본값: 현재 스크립트 디렉터리). 지정된 디렉터리 구조의 트리 보기를 표시합니다.

> 참고: `--input`과 `--url`은 상호 배타적이며, 하나는 제공해야 합니다.

## 환경 변수

명령줄 인수 대신 다음 환경 변수를 설정할 수 있습니다:

- `OPENAI_URL`: OpenAI API를 위한 URL.
- `API_KEY`: OpenAI API 키.
- `MODEL`: 사용할 OpenAI 모델 (예: `'gpt-3.5-turbo'`).

프로젝트 루트에 `.env` 파일을 설정하거나 셸에서 이를 내보낼 수 있습니다.
...
4. **URL의 Markdown 콘텐츠 번역:**

```bash
npx ai-markdown-translator -u https://gitee.com/h7ml/ai-markdown-translator/raw/main/README.md -o output.md -l "Italian"
```

5. **디렉터리에 있는 모든 Markdown 파일을 번역하고 이름 바꾸기:**

```bash
npx ai-markdown-translator -i ./markdown-files -l "Chinese" --rename
```

6. **Markdown 파일을 번역하고 출력 파일 이름 지정:**
...
- **이슈**: [이슈보고하기](https://github.com/h7ml/ai-markdown-translator/issues)
...
문제가 발생하거나 질문이 있는 경우 이 저장소에 이슈를 열어 주세요.